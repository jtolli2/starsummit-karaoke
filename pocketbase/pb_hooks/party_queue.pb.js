...
routerAdd('POST', '/api/karaoke/tablet/catalog/playlists/import', (c) => {
  ...
  if (!dryRun) {
    try {
      ...
      const ids = jsonValue(row, 'ordered_video_ids_json', []); if (JSON.stringify(ids) !== JSON.stringify(confirmation.orderedVideoIds || ids)) throw new Error('confirmation_binding_mismatch'); const expectedCounts = jsonValue(row, 'expected_counts_json', {}); if (JSON.stringify(expectedCounts) !== JSON.stringify(confirmation.expectedCounts || expectedCounts)) throw new Error('confirmation_binding_mismatch'); if (str(row, 'etag') && str(row, 'etag') !== String(confirmation.etag || '')) throw new Error('confirmation_binding_mismatch'); if (str(row, 'page_token') && String(input.pageToken || '') !== str(row, 'page_token')) throw new Error('confirmation_binding_mismatch'); if (num(row, 'import_cap') && Number(input.maxItems || 0) !== num(row, 'import_cap')) throw new Error('confirmation_binding_mismatch')
      if (str(row, 'owner_channel_id') !== String(confirmation.ownerChannelId || str(row, 'owner_channel_id'))) throw new Error('confirmation_binding_mismatch')
