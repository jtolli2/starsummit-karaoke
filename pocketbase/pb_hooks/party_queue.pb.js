...
routerAdd('POST', '/api/karaoke/tablet/catalog/review/selection', (c) => {
  ...
  try {
    ...
    const predicate = (song) => { const metadata = jsonValue(song, 'metadata_json', {}) || {}; let collision = []; try { collision = $app.findRecordsByFilter('karaoke_songs', 'identity_key = {:identity} && id != {:id}', '', 1, 0, { identity: str(song, 'identity_key'), id: id(song) }) } catch (_) {} return parser.isApprovable({ review_status: str(song, 'review_status'), identity_status: str(song, 'identity_status'), artist: str(song, 'artist'), title: str(song, 'title'), classification: str(song, 'classification'), eligible: song.getBool ? song.getBool('eligible') : false, embeddable: metadata.embeddable, available: metadata.privacyStatus === 'public' && metadata.uploadStatus === 'processed', eligibility_reason: str(song, 'eligibility_reason'), conflict: Boolean(str(song, 'conflict')) || collision.length > 0, alternatives_json: jsonValue(song, 'alternatives_json', []) }) }
