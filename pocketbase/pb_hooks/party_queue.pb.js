  ...
  try {
    ...
    $app.runInTransaction((tx) => {
      ...
      for (let itemIndex = 0; itemIndex < items.length; itemIndex++) {
        ...
        const title = String(item.title || youtubeId).slice(0, 240); const artist = String(item.artist || item.channelTitle || '').slice(0, 160); const result = classifyCatalogItem(item); const normalizedTitle = normalized(title, 240); const normalizedArtist = normalized(artist, 160); const identityKey = `${normalizedArtist}|${normalizedTitle}`
        ...
        if (identity) { const alternatives = identity.get ? (identity.get('alternatives_json') || []) : (identity.alternatives_json || []); const list = Array.isArray(alternatives) ? alternatives : []; if (!list.some((candidate) => String(candidate.youtubeId || '') === youtubeId)) { list.push({ youtubeId, title, artist, classification: result.classification, provenance: live ? 'youtube_api_import' : 'fixture_import', sourceUrl, sourceRetrievedAt: sourceRetrievedAt || now(), importedAt: now() }); set(identity, 'alternatives_json', list); tx.save(identity) } continue }
        set(song, 'youtube_id', youtubeId); set(song, 'title', title); set(song, 'artist', artist); set(song, 'provenance', live ? 'youtube_api_import' : 'fixture_import'); set(song, 'eligibility_reason', result.reason); set(song, 'source', live ? 'youtube' : 'fixture'); set(song, 'source_query', String(input.query || source.terms || '').slice(0, 160)); set(song, 'source_url', sourceUrl); set(song, 'source_retrieved_at', sourceRetrievedAt || now()); set(song, 'source_rank', offset + itemIndex + 1); set(song, 'source_terms', sourceTerms); set(song, 'classification', result.classification); set(song, 'classification_confidence', result.confidence); set(song, 'review_status', 'unreviewed'); set(song, 'eligible', false); set(song, 'normalized_title', normalizedTitle); set(song, 'normalized_artist', normalizedArtist); set(song, 'identity_key', identityKey); set(song, 'alternatives_json', []); set(song, 'review_history_json', []); set(song, 'metadata_json', { channelTitle: item.channelTitle || null, description: item.description || null, embeddable: item.embeddable === true, privacyStatus: item.privacyStatus || null, uploadStatus: item.uploadStatus || null, duration: item.duration || null, viewCount: item.viewCount || null }); set(song, 'import_batch', batchKey); set(song, 'imported_at', now()); diagnosticStage = 'song_save'; tx.save(song); imported++
      }
