...
routerAdd('GET', '/api/karaoke/tablet/catalog/checkpoint-health', (c) => {
  try { require(__hooks + '/party_queue.pb.js') } catch (_) {}
  const { auth, tablet, json, catalogCheckpointHealth, catalogFieldType, findPlaylistSnapshot } = globalThis.__partyQueue
  if (!tablet(auth(c))) return json(c, 403, 'forbidden', 'tablet_admin authentication required')
...
routerAdd('POST', '/api/karaoke/tablet/catalog/playlists/import', (c) => {
  try { require(__hooks + '/party_queue.pb.js') } catch (_) {}
  const { auth, tablet, json, body, now, future, id, str, num, set, setJson, jsonValue, classifyCatalogItem, recordYoutubeOperation, dayKey, hash, serializeJson, random, youtubeRequest, catalogFieldType, findPlaylistSnapshot } = globalThis.__partyQueue
  if (!tablet(auth(c))) return json(c, 403, 'forbidden', 'tablet_admin authentication required')
  ...
  try {
    ...
    importPhase = 'snapshot_verify'; const snapshotFingerprint = hash(serializeJson({ sourceKey, policyVersion: String(source.policyVersion || 'v1'), pageToken, rows }))
    ...
    importPhase = 'persist_results'; $app.runInTransaction((tx) => {
      ...
      if (snapshot) return
      importPhase = 'snapshot_identity_save'; snapshot = new Record(tx.findCollectionByNameOrId('karaoke_playlist_snapshots')); try { set(snapshot, 'source_key', sourceKey); set(snapshot, 'policy_version', String(source.policyVersion || 'v1')); set(snapshot, 'snapshot_fingerprint', snapshotFingerprint); tx.save(snapshot) } catch (_) { throw new Error('playlist_import_snapshot_identity_save_failed') }
