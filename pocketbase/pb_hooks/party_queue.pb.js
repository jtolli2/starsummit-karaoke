...
const TRUSTED_PLAYLIST_ELIGIBILITY_POLICY = 'native-playback-v2'
...
function parsePlaylistInput(raw) {
  ...
  if (!value || value.length > 512) throw new Error('playlist_input_invalid')
  if (/^(?:PL|UU|LL|FL|RD)[A-Za-z0-9_-]{16,}$/.test(value)) return { playlistId: value, sourceKey: value, input: value }
  ...
  if (!match) {
    ...
    if (/^https:\/\//i.test(value)) {
      ...
      if (host && !['youtube.com', 'www.youtube.com', 'm.youtube.com'].includes(host)) throw new Error('playlist_host_invalid')
    }
    throw new Error('playlist_url_invalid')
  }
  if (!['youtube.com', 'www.youtube.com', 'm.youtube.com'].includes(match[1].toLowerCase())) throw new Error('playlist_host_invalid')
  const params = {}; if (!match[3]) throw new Error('playlist_id_invalid')
  for (const pair of match[3].split('&')) {
    const pieces = pair.split('=')
    if (pieces.length !== 2 || !pieces[0] || Object.prototype.hasOwnProperty.call(params, pieces[0]) || !['list', 'index'].includes(pieces[0])) throw new Error('playlist_parameter_invalid')
    try { params[pieces[0]] = decodeURIComponent(pieces[1]) } catch (_) { throw new Error('playlist_url_invalid') }
  }
  if (!/^(?:PL|UU|LL|FL|RD)[A-Za-z0-9_-]{16,}$/.test(params.list || '')) throw new Error('playlist_id_invalid')
  return { playlistId: params.list, sourceKey: params.list, input: value }
}
function digest(value) { return hash(JSON.stringify(value)) }
function confirmationToken(payload, secret) {
  if (!secret) throw new Error('confirmation_secret_missing')
  if (typeof $security === 'undefined' || !$security.randomString) throw new Error('confirmation_crypto_unavailable')
  const randomValue = $security.randomString(64)
  return `pb1.${randomValue}`
}
const issueConfirmation = confirmationToken
function verifyConfirmation(token, secret) {
  if (!secret || typeof token !== 'string' || !/^pb1\.[A-Za-z0-9_-]{32,256}$/.test(token)) throw new Error('confirmation_invalid')
  return { opaque: true }
}
function isApprovable(song) {
  if (!song || String(song.review_status) !== 'needs_review' || !['verified_source', 'operator_corrected'].includes(String(song.identity_status))) return false
  if (!String(song.artist || '').trim() || !String(song.title || '').trim() || String(song.classification) !== 'karaoke') return false
  if (Number(song.classification_confidence || 0) < 0.8) return false
  ...
  return !(song.conflict || song.canonical_conflict || (Array.isArray(song.alternatives_json) && song.alternatives_json.length))
}
// One server-side policy is shared by individual, batch, and snapshot approval.
// Return a stable actionable code; never leak PocketBase/SQLite save internals.
function catalogApprovalReason(song, store) {
  if (!song) return 'song_not_found'
  const review = str(song, 'review_status'); const classification = str(song, 'classification')
  const wasApproved = review === 'approved'
  if (!['needs_review', 'approved'].includes(review)) return 'review_state_invalid'
  if (!['verified_source', 'operator_corrected'].includes(str(song, 'identity_status')) || !str(song, 'artist').trim() || !str(song, 'title').trim()) return 'missing_identity'
  if (classification !== 'karaoke') return 'non_karaoke'
  if (num(song, 'classification_confidence') < 0.8) return 'low_confidence'
  const alternatives = jsonValue(song, 'alternatives_json', null)
  if (alternatives !== null && !Array.isArray(alternatives)) return 'schema_invalid'
  if (Array.isArray(alternatives) && alternatives.length) return 'alternative_exists'
  const metadata = jsonValue(song, 'metadata_json', null)
  if (metadata !== null && (typeof metadata !== 'object' || Array.isArray(metadata))) return 'schema_invalid'
  if (['live', 'fallback', 'cover', 'tutorial', 'medley', 'mix', 'fallback_lyric', 'fallback_audio', 'policy_rejected', 'unavailable', 'region_blocked', 'non_embeddable'].includes(str(song, 'eligibility_reason'))) return 'unavailable'
  if (metadata && (metadata.privacyStatus !== undefined || metadata.uploadStatus !== undefined) && (metadata.privacyStatus !== 'public' || metadata.uploadStatus !== 'processed')) return 'unavailable'
  try {
    if (store && str(song, 'identity_key')) {
      const collision = store.findRecordsByFilter('karaoke_songs', 'identity_key = {:identity} && id != {:id}', '', 1, 0, { identity: str(song, 'identity_key'), id: id(song) })
      if (collision.length) return 'identity_conflict'
    }
  } catch (_) { return 'schema_invalid' }
  if (wasApproved) return 'already_approved'
  return null
}
...
function findPlaylistSnapshot(db, sourceKey, pageToken, fingerprint) {
  const targetPage = String(pageToken || '')
  const rows = db.findRecordsByFilter('karaoke_playlist_snapshots', 'source_key = {:source} && snapshot_fingerprint = {:fingerprint}', '+id', 3, 0, { source: sourceKey, fingerprint })
  if ((rows || []).length >= 3) throw new Error('playlist_snapshot_ambiguous')
  const matches = (rows || []).filter((row) => String(str(row, 'page_token') || '') === targetPage)
  if (matches.length > 1) throw new Error('playlist_snapshot_ambiguous')
  return matches[0] || null
}
function classifyTrustedVideoAvailability(video) {
  if (!video) return { unavailable: true, metadataMissing: 1, nonEmbeddable: 0 }
  const status = video.status || {}
  return { unavailable: status.privacyStatus !== 'public' || status.uploadStatus !== 'processed', metadataMissing: 0, nonEmbeddable: status.embeddable === true ? 0 : 1 }
}
...
routerAdd('POST', '/api/karaoke/parties/songs/fallback/request', (c) => {
  ...
  try {
    ...
    return c.json(replayed ? 200 : 201, result)
  } catch (error) {
    ...
    if (reason.toLowerCase().includes('unique')) {
      ...
      if (['party_expired', 'fallback_candidate_unavailable', 'rate_limited', 'duplicate_song'].includes(retryReason)) return q.json(c, { party_expired: 410, fallback_candidate_unavailable: 422, rate_limited: 429, duplicate_song: 409 }[retryReason], retryReason, retryReason === 'duplicate_song' ? 'Song is already queued' : 'Fallback request rejected')
      return q.json(c, 503, 'fallback_persistence_unavailable', 'Fallback request could not be saved; retry shortly')
    }
    const known = { party_expired: [410, 'Fallback request rejected'], duplicate_song: [409, 'Song is already queued'], fallback_candidate_unavailable: [422, 'Fallback candidate is no longer available'], rate_limited: [429, 'Fallback request rate limit reached'] }
    if (known[reason]) return q.json(c, known[reason][0], reason, known[reason][1])
    // Do not expose PocketBase/SQLite validation details to guests. Keep the
    // server-side error available in logs while returning a stable retryable
    // contract for schema or persistence failures.
    try { console.error(`Fallback request persistence failure (reason=${reason.slice(0, 120)})`) } catch (_) {}
    return q.json(c, 503, 'fallback_persistence_unavailable', 'Fallback request could not be saved; retry shortly')
  }
})
...
routerAdd('GET', '/api/karaoke/tablet/catalog/checkpoint-health', (c) => {
  try { require(__hooks + '/party_queue.pb.js') } catch (_) {}
  const { auth, tablet, json, catalogCheckpointHealth, catalogFieldType, findPlaylistSnapshot, classifyTrustedVideoAvailability } = globalThis.__partyQueue
  if (!tablet(auth(c))) return json(c, 403, 'forbidden', 'tablet_admin authentication required')
...
routerAdd('POST', '/api/karaoke/tablet/catalog/playlists/import', (c) => {
  ...
  const input = body(c)
  let sourceKey = String(input.sourceKey || '')
  let arbitraryPlaylistId = String(input.playlistId || '')
  if (!sourceKey && (input.playlistUrl || arbitraryPlaylistId)) {
    let parsed
    try {
      const parser = globalThis.__partyQueue
      parsed = parser.parsePlaylistInput(input.playlistUrl || arbitraryPlaylistId)
    } catch (error) { return json(c, 422, String(error.message || 'playlist_input_invalid'), 'Playlist URL or ID is invalid') }
    arbitraryPlaylistId = parsed.playlistId; sourceKey = parsed.sourceKey
  }
  // Confirmed imports may intentionally submit only the opaque token and
  // snapshot. Reconstruct the immutable source/page/cap binding before any
  // source-key validation; browser fields are never trusted for these values.
  if (!sourceKey && input.confirmationToken) {
    try {
      const digest = hash(String(input.confirmationToken)); const confirmationRow = $app.findFirstRecordByFilter('karaoke_playlist_confirmations', 'token_digest = {:digest}', { digest })
      if (confirmationRow && str(confirmationRow, 'admin_id') === id(auth(c))) {
        sourceKey = str(confirmationRow, 'playlist_id'); arbitraryPlaylistId = sourceKey
        const boundPage = str(confirmationRow, 'page_token'); const boundCap = num(confirmationRow, 'import_cap')
        if (boundPage && input.pageToken === undefined) input.pageToken = boundPage
        if (boundCap && input.maxItems === undefined) input.maxItems = boundCap
      }
    } catch (_) {}
    try { const encoded = String(input.confirmationToken).split('.')[0]; const bound = JSON.parse(Buffer.from(encoded, 'base64url').toString('utf8')); if (!sourceKey && bound.playlistId) sourceKey = String(bound.playlistId); if (input.pageToken === undefined && bound.pageToken) input.pageToken = String(bound.pageToken); if (input.maxItems === undefined && bound.importCap) input.maxItems = Number(bound.importCap) } catch (_) {}
  }
  const legacySource = /^UC[A-Za-z0-9_-]{20,}:(?:PL|UU|LL|FL|RD)[A-Za-z0-9_-]{16,}$/.test(sourceKey)
  if (!legacySource && !/^(?:PL|UU|LL|FL|RD)[A-Za-z0-9_-]{16,}$/.test(sourceKey)) return json(c, 422, 'playlist_source_key_invalid', 'Playlist identity is invalid')
  let allowlist = []
  try { allowlist = JSON.parse(String($os.getenv('KARAOKE_PLAYLIST_ALLOWLIST_JSON') || '[]')) } catch (_) { if (legacySource) return json(c, 503, 'playlist_allowlist_invalid', 'Playlist allowlist is not configured') }
  if ((!Array.isArray(allowlist) || allowlist.length > 12) && legacySource) return json(c, 503, 'playlist_allowlist_invalid', 'Playlist allowlist is not configured')
  ...
  if (!source && !legacySource) source = { playlistId: sourceKey, channelId: '', channelName: '', playlistName: '', rationale: '', policyVersion: 'public-v1', known: false }
  if (!source || !/^(?:PL|UU|LL|FL|RD)[A-Za-z0-9_-]{16,}$/.test(String(source.playlistId || ''))) return json(c, 422, 'playlist_source_not_allowed', 'Playlist source is not configured or confirmed')
  const sourcePolicy = String(source.policyVersion || 'v1'); if (!/^[A-Za-z0-9._-]{1,16}$/.test(sourcePolicy)) return json(c, 503, 'playlist_allowlist_invalid', 'Playlist allowlist is not configured'); const maxItems = Math.max(1, Math.min(50, Number(input.maxItems || 25) || 25)); const pageToken = String(input.pageToken || '').slice(0, 240); const expectedSnapshot = String(input.snapshotFingerprint || ''); const dryRun = input.dryRun !== false; const revalidate = input.revalidate === true; const policyVersion = `${sourcePolicy}:${TRUSTED_PLAYLIST_ELIGIBILITY_POLICY}`; const claimKey = `playlist:${sourceKey}:${policyVersion}:${pageToken}:${maxItems}`
  let confirmation = null
  let persistedConfirmation = null
  if (!dryRun) {
    try {
      const parser = globalThis.__partyQueue; const secret = String($os.getenv('PLAYLIST_CONFIRMATION_SECRET') || $os.getenv('YOUTUBE_API_KEY') || '')
      confirmation = parser.verifyConfirmation(input.confirmationToken, secret, { adminId: id(auth(c)), playlistId: source.playlistId, snapshot: expectedSnapshot, snapshotFingerprint: expectedSnapshot, policyVersion }); if (confirmation.pageToken !== undefined && String(input.pageToken || '') !== String(confirmation.pageToken || '')) throw new Error('confirmation_binding_mismatch'); if (confirmation.importCap !== undefined && Number(input.maxItems || 0) !== Number(confirmation.importCap || 0)) throw new Error('confirmation_binding_mismatch')
      const digest = hash(String(input.confirmationToken || '')); const row = $app.findFirstRecordByFilter('karaoke_playlist_confirmations', 'token_digest = {:digest}', { digest }); persistedConfirmation = row; if (!row || ['rejected', 'expired'].includes(str(row, 'status')) || new Date(str(row, 'expires_at')).getTime() < Date.now()) throw new Error('confirmation_expired'); if (str(row, 'admin_id') !== id(auth(c))) throw new Error('confirmation_binding_mismatch'); if (str(row, 'status') === 'used') { let completed = false; try { const replayClaim = $app.findFirstRecordByFilter('karaoke_playlist_claims', 'claim_key = {:key}', { key: claimKey }); completed = Boolean(replayClaim && ['ready', 'complete'].includes(str(replayClaim, 'status')) && jsonValue(replayClaim, 'payload_json', null)) } catch (_) {} if (!completed) throw new Error('confirmation_replay_unavailable'); return c.json(200, { sourceKey, imported: 0, duplicates: 0, unavailable: 0, replay: true, snapshotFingerprint: expectedSnapshot }) }
      const ids = jsonValue(row, 'ordered_video_ids_json', []); const expectedCounts = jsonValue(row, 'expected_counts_json', {}); const rowEtag = str(row, 'etag'); const rowPage = str(row, 'page_token'); const rowCap = num(row, 'import_cap'); const rowOwner = str(row, 'owner_channel_id'); const rowVisibility = str(row, 'visibility');
      // Every persisted binding is checked directly; opaque worker tokens do
      // not provide claims and must never be treated as proof.
      if (str(row, 'playlist_id') !== String(source.playlistId) || str(row, 'snapshot_fingerprint') !== expectedSnapshot || str(row, 'policy_version') !== policyVersion || !rowOwner || rowVisibility !== 'public' || String(input.pageToken || '') !== rowPage || Number(input.maxItems || 0) !== rowCap || !Array.isArray(ids) || !expectedCounts || typeof expectedCounts !== 'object') throw new Error('confirmation_binding_mismatch')
      if (!confirmation.opaque) { if (confirmation.pageToken !== undefined && String(rowPage) !== String(confirmation.pageToken)) throw new Error('confirmation_binding_mismatch'); if (confirmation.importCap !== undefined && Number(rowCap) !== Number(confirmation.importCap)) throw new Error('confirmation_binding_mismatch'); if (JSON.stringify(ids) !== JSON.stringify(confirmation.orderedVideoIds || [])) throw new Error('confirmation_binding_mismatch'); if (JSON.stringify(expectedCounts) !== JSON.stringify(confirmation.expectedCounts || {})) throw new Error('confirmation_binding_mismatch'); if (String(rowEtag) !== String(confirmation.etag || '')) throw new Error('confirmation_binding_mismatch'); if (String(rowOwner) !== String(confirmation.ownerChannelId || '')) throw new Error('confirmation_binding_mismatch') }
    } catch (error) { const code = String(error.message || 'confirmation_invalid'); return json(c, 409, code, code === 'confirmation_expired' ? 'Preview confirmation expired' : 'A valid admin preview confirmation is required') }
  }
  try { const snapshots = $app.findCollectionByNameOrId('karaoke_playlist_snapshots'); const expected = [['source_key', 'text'], ['policy_version', 'text'], ['page_token', 'text'], ['next_page_token', 'text'], ['snapshot_fingerprint', 'text'], ['metadata_digest', 'text'], ['ordered_video_ids_json', 'json'], ['retrieved_at', 'date'], ['owner_validated_at', 'date']]; if (!snapshots || expected.some(([name, type]) => catalogFieldType(snapshots.fields?.getByName(name)) !== type)) return json(c, 503, 'playlist_snapshot_schema_mismatch', 'Trusted playlist snapshot storage is unavailable') } catch (_) { return json(c, 503, 'playlist_snapshot_schema_mismatch', 'Trusted playlist snapshot storage is unavailable') }
  if (String(input.pageToken || '').length > 240) return json(c, 422, 'playlist_snapshot_token_too_long', 'The trusted playlist page token is invalid')
  ...
  if (!dryRun || revalidate) { try { priorSnapshot = findPlaylistSnapshot($app, sourceKey, pageToken, expectedSnapshot) } catch (error) { const code = String(error.message || '') === 'playlist_snapshot_ambiguous' ? 'playlist_snapshot_ambiguous' : 'playlist_snapshot_lookup_failed'; return json(c, code === 'playlist_snapshot_ambiguous' ? 409 : 503, code, code === 'playlist_snapshot_ambiguous' ? 'The retained playlist snapshot is ambiguous' : 'Trusted playlist snapshot lookup is unavailable') } if (!priorSnapshot && revalidate) return json(c, 409, 'playlist_snapshot_not_found', 'The retained playlist snapshot is unavailable'); if (priorSnapshot) { const validated = new Date(str(priorSnapshot, 'owner_validated_at') || str(priorSnapshot, 'retrieved_at')).getTime(); const usedConfirmation = Boolean(persistedConfirmation && str(persistedConfirmation, 'status') === 'used'); if ((!revalidate && dryRun) || (!revalidate && usedConfirmation)) { // Replay is safe only after the retained snapshot proves every confirmation binding.
        if (persistedConfirmation) { const snapIds = jsonValue(priorSnapshot, 'ordered_video_ids_json', []); const boundIds = jsonValue(persistedConfirmation, 'ordered_video_ids_json', []); const retainedEtag = str(priorSnapshot, 'etag'); if (str(persistedConfirmation, 'playlist_id') !== source.playlistId || str(persistedConfirmation, 'snapshot_fingerprint') !== expectedSnapshot || str(persistedConfirmation, 'policy_version') !== policyVersion || !str(persistedConfirmation, 'owner_channel_id') || str(persistedConfirmation, 'visibility') !== 'public' || str(persistedConfirmation, 'etag') !== retainedEtag || String(str(persistedConfirmation, 'page_token')) !== String(pageToken) || Number(num(persistedConfirmation, 'import_cap')) !== Number(maxItems) || JSON.stringify(boundIds) !== JSON.stringify(snapIds)) return json(c, 409, 'confirmation_binding_mismatch', 'A valid admin preview confirmation is required') }
        let breakdown = {}; try { const replayClaim = $app.findFirstRecordByFilter('karaoke_playlist_claims', 'claim_key = {:key}', { key: claimKey }); breakdown = jsonValue(replayClaim, 'payload_json', {})?.unavailableReasons || {} } catch (_) {} return c.json(200, { sourceKey, imported: 0, unavailable: Number(breakdown.total || 0), unavailableReasons: breakdown, duplicates: 0, replay: true, nextPageToken: str(priorSnapshot, 'next_page_token') || '', snapshotFingerprint: expectedSnapshot }) } } }
  ...
  if (revalidate) {
    ...
    try {
      ...
      const videos = new Map((metadata.items || []).map((video) => [String(video.id || ''), video])); ids.forEach((videoId) => { const video = videos.get(videoId); const verdict = classifyTrustedVideoAvailability(video); if (verdict.metadataMissing) unavailableReasons.metadataMissing += verdict.metadataMissing; if (verdict.unavailable) unavailableReasons.total++; if (verdict.nonEmbeddable) unavailableReasons.nonEmbeddable += verdict.nonEmbeddable; if (!video) return; const status = video.status || {}; const privacy = String(status.privacyStatus || 'missing'); if (privacy !== 'public') unavailableReasons.privacy[privacy in unavailableReasons.privacy ? privacy : 'other']++; const upload = String(status.uploadStatus || 'missing'); if (upload !== 'processed') unavailableReasons.uploadStatus[upload in unavailableReasons.uploadStatus ? upload : 'other']++ })
  ...
  if (cachedPreview) {
    // A cached immutable preview is quota-free, but confirmations are
    // single-purpose. Mint and persist a fresh ten-minute token for replay.
    try {
      const parser = globalThis.__partyQueue
      const secret = String($os.getenv('PLAYLIST_CONFIRMATION_SECRET') || $os.getenv('YOUTUBE_API_KEY') || '')
      const token = parser.issueConfirmation({ adminId: id(auth(c)), playlistId: cachedPreview.playlistId || source.playlistId, snapshot: cachedPreview.snapshotFingerprint, snapshotFingerprint: cachedPreview.snapshotFingerprint, ownerChannelId: cachedPreview.ownerChannelId, visibility: cachedPreview.visibility, orderedVideoIds: cachedPreview.orderedVideoIds || [], etag: cachedPreview.etag || '', expectedCounts: cachedPreview.expectedCounts || {}, importCap: Number(cachedPreview.importCap || maxItems), pageToken: cachedPreview.pageToken || pageToken, policyVersion }, secret)
      $app.runInTransaction((tx) => { const row = new Record(tx.findCollectionByNameOrId('karaoke_playlist_confirmations')); set(row, 'token_digest', hash(token)); set(row, 'admin_id', id(auth(c))); set(row, 'playlist_id', cachedPreview.playlistId || source.playlistId); set(row, 'owner_channel_id', cachedPreview.ownerChannelId || ''); set(row, 'visibility', cachedPreview.visibility || 'public'); set(row, 'snapshot_fingerprint', cachedPreview.snapshotFingerprint); setJson(row, 'ordered_video_ids_json', cachedPreview.orderedVideoIds || []); setJson(row, 'expected_counts_json', cachedPreview.expectedCounts || {}); set(row, 'policy_version', policyVersion); set(row, 'etag', cachedPreview.etag || ''); set(row, 'page_token', cachedPreview.pageToken || pageToken); set(row, 'import_cap', Number(cachedPreview.importCap || maxItems)); set(row, 'expires_at', future(600000)); set(row, 'status', 'issued'); tx.save(row) })
      return c.json(200, { ...cachedPreview, confirmationToken: token, confirmationExpiresAt: future(600000), replay: true, quota: { ...(cachedPreview.quota || {}), cached: true } })
    } catch (_) { return json(c, 503, 'confirmation_issue_failed', 'Preview confirmation could not be issued') }
  }
  ...
  const attempts = { playlistsList: 0, playlistItemsList: 0, videosList: 0 }; let importPhase = 'reserve'
  ...
  try {
    ...
    importPhase = 'owner_fetch'; const playlist = call('playlistsList', `playlists?part=snippet,status&id=${encodeURIComponent(source.playlistId)}`).items?.[0]
    if (!playlist || playlist.status?.privacyStatus !== 'public') throw new Error('playlist_unavailable')
    const verifiedOwnerId = String(playlist.snippet?.channelId || '')
    if (source.channelId && verifiedOwnerId !== String(source.channelId)) throw new Error('playlist_owner_mismatch')
    if (!source.channelId && /^UC[A-Za-z0-9_-]{20,}$/.test(verifiedOwnerId)) source.channelId = verifiedOwnerId
    ...
    importPhase = 'playlist_items_fetch'; const page = call('playlistItemsList', `playlistItems?part=snippet,contentDetails,status&playlistId=${encodeURIComponent(source.playlistId)}&maxResults=${maxItems}${pageToken ? `&pageToken=${encodeURIComponent(pageToken)}` : ''}`)
    ...
    importPhase = 'snapshot_verify'; const snapshotFingerprint = hash(serializeJson({ sourceKey, policyVersion, pageToken, rows }))
    if (String(page.nextPageToken || '').length > 240) throw new Error('playlist_snapshot_token_too_long')
    ...
    // Revalidate every persisted confirmation against the authoritative API
    // response before videos are fetched or any catalog song can be mutated.
    // This also prevents an unknown-owner confirmation from being overwritten
    // by a later playlist response with different ownership metadata.
    if (!dryRun) {
      const currentOwner = String(playlist.snippet?.channelId || '')
      const currentVisibility = String(playlist.status?.privacyStatus || '')
      const currentEtag = String(page.etag || playlist.etag || '')
      const boundIds = jsonValue(persistedConfirmation, 'ordered_video_ids_json', [])
      const boundPage = str(persistedConfirmation, 'page_token')
      const boundCap = num(persistedConfirmation, 'import_cap')
      if (!persistedConfirmation || currentVisibility !== 'public' || !currentOwner || currentOwner !== str(persistedConfirmation, 'owner_channel_id') || currentVisibility !== str(persistedConfirmation, 'visibility') || currentEtag !== str(persistedConfirmation, 'etag') || String(pageToken) !== String(boundPage || '') || Number(maxItems) !== Number(boundCap || 0) || JSON.stringify(rows.map((row) => row.youtubeId)) !== JSON.stringify(Array.isArray(boundIds) ? boundIds : [])) throw new Error('playlist_confirmation_stale')
    }
    if (dryRun) { const ownerChannelId = String(playlist.snippet?.channelId || ''); const preview = { source: { sourceKey, channelId: ownerChannelId, playlistId: source.playlistId, channelName: String(playlist.snippet?.channelTitle || source.channelName || ''), playlistName: String(playlist.snippet?.title || source.playlistName || ''), rationale: source.rationale || '', known: Boolean(source.known), policyVersion }, playlistId: source.playlistId, ownerChannelId, ownerChannelTitle: String(playlist.snippet?.channelTitle || ''), visibility: String(playlist.status?.privacyStatus || ''), etag: String(page.etag || playlist.etag || ''), expectedItems: rows.length, itemCount: rows.length, hiddenUnavailableCount: 0, pageToken, nextPageToken: page.nextPageToken || '', snapshotFingerprint, orderedVideoIds: rows.map((row) => row.youtubeId), expectedCounts: { items: rows.length, new: rows.length, duplicates: 0, unavailable: 0 }, importCap: maxItems, modeledCost: modeled, expectedQuota: modeled.total, apiOperations: modeled, ownerValidatedAt: now(), retrievalTime: now(), pageCount: 1, duplicates: 0, knownParser: Boolean(source.known), identityWarning: 'Uploader and title metadata are not canonical song identity.' }; $app.runInTransaction((tx) => { const claim = tx.findFirstRecordByFilter('karaoke_playlist_claims', 'claim_key = {:key}', { key: claimKey }); if (str(claim, 'owner_token') !== ownerToken) throw new Error('playlist_claim_stale'); const actual = attempts.playlistsList + attempts.playlistItemsList; const reserved = num(claim, 'reserved_units'); const quota = tx.findFirstRecordByFilter('karaoke_youtube_quota', 'day_key = {:day}', { day: str(claim, 'quota_day_key') }); set(quota, 'reserved', Math.max(0, num(quota, 'reserved') - reserved)); set(quota, 'spent', num(quota, 'spent') + actual); tx.save(quota); set(claim, 'reserved_units', 0); set(claim, 'spent_units', num(claim, 'spent_units') + actual); set(claim, 'status', 'ready'); const secret = String($os.getenv('PLAYLIST_CONFIRMATION_SECRET') || $os.getenv('YOUTUBE_API_KEY') || ''); const parser = globalThis.__partyQueue; const token = parser.issueConfirmation({ adminId: id(auth(c)), playlistId: source.playlistId, snapshot: snapshotFingerprint, snapshotFingerprint, ownerChannelId, visibility: preview.visibility, orderedVideoIds: preview.orderedVideoIds, etag: preview.etag, expectedCounts: preview.expectedCounts, importCap: maxItems, pageToken, policyVersion }, secret); const confirmation = new Record(tx.findCollectionByNameOrId('karaoke_playlist_confirmations')); set(confirmation, 'token_digest', hash(token)); set(confirmation, 'admin_id', id(auth(c))); set(confirmation, 'playlist_id', source.playlistId); set(confirmation, 'owner_channel_id', ownerChannelId); set(confirmation, 'visibility', preview.visibility); set(confirmation, 'snapshot_fingerprint', snapshotFingerprint); setJson(confirmation, 'ordered_video_ids_json', preview.orderedVideoIds); setJson(confirmation, 'expected_counts_json', preview.expectedCounts); set(confirmation, 'policy_version', policyVersion); set(confirmation, 'etag', preview.etag); set(confirmation, 'page_token', pageToken); set(confirmation, 'import_cap', maxItems); set(confirmation, 'expires_at', future(600000)); set(confirmation, 'status', 'issued'); tx.save(confirmation); preview.confirmationToken = token; preview.confirmationExpiresAt = str(confirmation, 'expires_at'); setJson(claim, 'payload_json', preview); tx.save(claim); recordYoutubeOperation(tx, 'playlists.list', sourceKey, snapshotFingerprint, 1, attempts.playlistsList); recordYoutubeOperation(tx, 'playlistItems.list', sourceKey, snapshotFingerprint, 1, attempts.playlistItemsList) }); return c.json(200, preview) }
    importPhase = 'video_metadata_fetch'; const metadata = rows.length ? call('videosList', `videos?part=snippet,contentDetails,status&id=${encodeURIComponent(rows.map((row) => row.youtubeId).join(','))}`) : { items: [] }
    ...
    importPhase = 'persist_results'; $app.runInTransaction((tx) => {
      let snapshot = null; try { snapshot = findPlaylistSnapshot(tx, sourceKey, pageToken, snapshotFingerprint) } catch (error) { throw new Error(String(error.message || '') === 'playlist_snapshot_ambiguous' ? 'playlist_snapshot_ambiguous' : 'playlist_snapshot_lookup_failed') }
      if (snapshot) return
      importPhase = 'snapshot_identity_save'; snapshot = new Record(tx.findCollectionByNameOrId('karaoke_playlist_snapshots')); try { set(snapshot, 'source_key', sourceKey); set(snapshot, 'policy_version', policyVersion); set(snapshot, 'snapshot_fingerprint', snapshotFingerprint); tx.save(snapshot) } catch (_) { throw new Error('playlist_import_snapshot_identity_save_failed') }
      importPhase = 'snapshot_page_save'; try { set(snapshot, 'page_token', pageToken); set(snapshot, 'next_page_token', String(page.nextPageToken || '')); tx.save(snapshot) } catch (_) { throw new Error('playlist_import_snapshot_page_save_failed') }
      importPhase = 'snapshot_digest_save'; try { set(snapshot, 'metadata_digest', metadataDigest); tx.save(snapshot) } catch (_) { throw new Error('playlist_import_snapshot_digest_save_failed') }
      importPhase = 'snapshot_ids_save'; try { setJson(snapshot, 'ordered_video_ids_json', rows.map((row) => row.youtubeId)); tx.save(snapshot) } catch (_) { throw new Error('playlist_import_snapshot_ids_save_failed') }
      importPhase = 'snapshot_dates_save'; try { set(snapshot, 'retrieved_at', now()); set(snapshot, 'owner_validated_at', now()); tx.save(snapshot) } catch (_) { throw new Error('playlist_import_snapshot_dates_save_failed') }
      importPhase = 'song_save'; for (const row of rows) {
        const video = videos.get(row.youtubeId); const verdict = classifyTrustedVideoAvailability(video); const status = video?.status || {}; const snippet = video?.snippet || {}; const embeddable = status.embeddable === true; if (verdict.unavailable) { unavailable++; unavailableReasons.total++; if (verdict.metadataMissing) unavailableReasons.metadataMissing += verdict.metadataMissing; if (!video) continue; if (verdict.nonEmbeddable) unavailableReasons.nonEmbeddable += verdict.nonEmbeddable; const privacy = String(status.privacyStatus || 'missing'); if (privacy !== 'public') unavailableReasons.privacy[privacy in unavailableReasons.privacy ? privacy : 'other']++; const upload = String(status.uploadStatus || 'missing'); if (upload !== 'processed') unavailableReasons.uploadStatus[upload in unavailableReasons.uploadStatus ? upload : 'other']++; continue }
        if (verdict.nonEmbeddable) unavailableReasons.nonEmbeddable += verdict.nonEmbeddable
      ...
      try { importPhase = 'settle_success'; recordYoutubeOperation(tx, 'playlists.list', sourceKey, snapshotFingerprint, 1, attempts.playlistsList); recordYoutubeOperation(tx, 'playlistItems.list', sourceKey, snapshotFingerprint, 1, attempts.playlistItemsList); if (modeled.videosList) recordYoutubeOperation(tx, 'videos.list', sourceKey, snapshotFingerprint, modeled.videosList, attempts.videosList); const actual = attempts.playlistsList + attempts.playlistItemsList + attempts.videosList; const claim = tx.findFirstRecordByFilter('karaoke_playlist_claims', 'claim_key = {:key}', { key: claimKey }); if (str(claim, 'owner_token') !== ownerToken) throw new Error('playlist_claim_stale'); const reserved = num(claim, 'reserved_units'); const quota = tx.findFirstRecordByFilter('karaoke_youtube_quota', 'day_key = {:day}', { day: str(claim, 'quota_day_key') }); set(quota, 'reserved', Math.max(0, num(quota, 'reserved') - reserved)); set(quota, 'spent', num(quota, 'spent') + actual); tx.save(quota); set(claim, 'reserved_units', 0); set(claim, 'spent_units', num(claim, 'spent_units') + actual); set(claim, 'status', 'ready'); setJson(claim, 'payload_json', { sourceKey, pageToken, nextPageToken: page.nextPageToken || '', snapshotFingerprint, metadataDigest, unavailableReasons }); tx.save(claim); if (!dryRun && input.confirmationToken) { const row = tx.findFirstRecordByFilter('karaoke_playlist_confirmations', 'token_digest = {:digest}', { digest: hash(String(input.confirmationToken)) }); if (row) { set(row, 'status', 'used'); set(row, 'used_at', now()); tx.save(row) } } } catch (_) { throw new Error('playlist_import_settle_success_failed') }
    })
    importPhase = 'settle_success'; return c.json(200, { sourceKey, imported, unavailable, unavailableReasons, duplicates, replay: imported === 0 && duplicates === 0 && unavailable === 0, nextPageToken: page.nextPageToken || '', snapshotFingerprint, metadataDigest, modeledCost: modeled })
  } catch (error) { const message = String(error.message || ''); const upstream = /^(?:youtube_http_[1-5]\d\d|youtube_http_unavailable|youtube_network_error|youtube_request_failed)$/.test(message) ? message : ''; const phaseCodes = ['owner_fetch', 'playlist_items_fetch', 'snapshot_verify', 'video_metadata_fetch', 'persist_results', 'snapshot_save', 'snapshot_identity_save', 'snapshot_page_save', 'snapshot_digest_save', 'snapshot_ids_save', 'snapshot_dates_save', 'song_save', 'settle_success'].map((phase) => `playlist_import_${phase}_failed`); const phaseFailure = phaseCodes.includes(message) ? message : phaseCodes.includes(`playlist_import_${importPhase}_failed`) ? `playlist_import_${importPhase}_failed` : 'playlist_import_persist_results_failed'; const auditCode = ['playlist_owner_mismatch', 'playlist_confirmation_stale', 'playlist_unavailable', 'playlist_preview_stale', 'playlist_snapshot_token_too_long', 'playlist_snapshot_data_invalid', 'playlist_snapshot_ambiguous', 'playlist_snapshot_lookup_failed'].includes(message) ? message : upstream || phaseFailure; const observed = attempts.playlistsList + attempts.playlistItemsList + attempts.videosList; try { $app.runInTransaction((tx) => { const claim = tx.findFirstRecordByFilter('karaoke_playlist_claims', 'claim_key = {:key}', { key: claimKey }); if (claim && str(claim, 'owner_token') === ownerToken) { const reserved = num(claim, 'reserved_units'); const quota = tx.findFirstRecordByFilter('karaoke_youtube_quota', 'day_key = {:day}', { day: str(claim, 'quota_day_key') }); set(quota, 'reserved', Math.max(0, num(quota, 'reserved') - reserved)); set(quota, 'spent', num(quota, 'spent') + observed); tx.save(quota); set(claim, 'reserved_units', 0); set(claim, 'spent_units', num(claim, 'spent_units') + observed); set(claim, 'status', 'failed'); set(claim, 'error_code', auditCode); tx.save(claim) } }) } catch (_) {} const code = ['playlist_owner_mismatch', 'playlist_confirmation_stale', 'playlist_unavailable', 'playlist_preview_stale', 'playlist_snapshot_token_too_long', 'playlist_snapshot_data_invalid', 'playlist_snapshot_ambiguous', 'playlist_snapshot_lookup_failed'].includes(message) ? message : upstream || phaseFailure; return json(c, code === 'playlist_owner_mismatch' || code === 'playlist_confirmation_stale' || code === 'playlist_unavailable' || code === 'playlist_preview_stale' ? (code === 'playlist_unavailable' ? 404 : 409) : upstream ? 502 : 503, code, code === 'playlist_owner_mismatch' ? 'Playlist ownership does not match the trusted source allowlist' : code === 'playlist_confirmation_stale' ? 'Playlist changed since preview; review it again before importing' : code === 'playlist_unavailable' ? 'The trusted playlist is unavailable or no longer public' : code === 'playlist_preview_stale' ? 'Playlist changed since preview; review it again before importing' : upstream ? 'Trusted playlist source is temporarily unavailable' : 'Trusted playlist import failed') }
})

// Issue a durable confirmation for an already-fetched preview snapshot. This
// endpoint is intentionally separate so previewing never imports.
// Compatibility aliases are kept explicit; clients should use the canonical
// import/confirm routes when they need the full upstream preview payload.
routerAdd('POST', '/api/karaoke/tablet/catalog/playlists/preview', (c) => {
  try { require(__hooks + '/party_queue.pb.js') } catch (_) {}
  const { auth, tablet, json } = globalThis.__partyQueue
  if (!tablet(auth(c))) return json(c, 403, 'forbidden', 'tablet_admin authentication required')
  return json(c, 409, 'preview_route_alias', 'Use /api/karaoke/tablet/catalog/playlists/import with dryRun=true')
})

routerAdd('POST', '/api/karaoke/tablet/catalog/playlists/import-confirmed', (c) => {
  try { require(__hooks + '/party_queue.pb.js') } catch (_) {}
  const { auth, tablet, json } = globalThis.__partyQueue
  if (!tablet(auth(c))) return json(c, 403, 'forbidden', 'tablet_admin authentication required')
  return json(c, 409, 'import_route_alias', 'Use /api/karaoke/tablet/catalog/playlists/import with confirmationToken')
})

routerAdd('POST', '/api/karaoke/tablet/catalog/playlists/confirm', (c) => {
  try { require(__hooks + '/party_queue.pb.js') } catch (_) {}
  const { auth, tablet, json, body, id, hash, set, setJson, future, str } = globalThis.__partyQueue
  if (!tablet(auth(c))) return json(c, 403, 'forbidden', 'tablet_admin authentication required')
  const input = body(c); const playlistId = String(input.playlistId || ''); const snapshot = String(input.snapshotFingerprint || ''); if (!/^(?:PL|UU|LL|FL|RD)[A-Za-z0-9_-]{16,}$/.test(playlistId) || !/^[a-f0-9]{64}$/i.test(snapshot)) return json(c, 422, 'confirmation_binding_invalid', 'Playlist snapshot binding is invalid')
  try { let policyVersion = ''; let preview = null; try { const snap = $app.findFirstRecordByFilter('karaoke_playlist_snapshots', 'snapshot_fingerprint = {:snapshot}', { snapshot }); policyVersion = snap ? str(snap, 'policy_version') : ''; const claims = $app.findRecordsByFilter('karaoke_playlist_claims', 'payload_json ~ {:needle}', '', 20, 0, { needle: snapshot }); for (const claim of claims) { const payload = jsonValue(claim, 'payload_json', {}); if (payload && payload.snapshotFingerprint === snapshot) { preview = payload; break } } } catch (_) {} if (!policyVersion || !preview) throw new Error('confirmation_binding_invalid'); const parser = globalThis.__partyQueue; const secret = String($os.getenv('PLAYLIST_CONFIRMATION_SECRET') || $os.getenv('YOUTUBE_API_KEY') || ''); const ownerChannelId = String(preview.ownerChannelId || preview.source?.channelId || ''); const token = parser.issueConfirmation({ adminId: id(auth(c)), playlistId, snapshot, snapshotFingerprint: snapshot, policyVersion, ownerChannelId, visibility: String(preview.visibility || 'public'), orderedVideoIds: preview.orderedVideoIds || [], etag: String(preview.etag || ''), expectedCounts: preview.expectedCounts || {}, importCap: Number(preview.importCap || 0), pageToken: String(preview.pageToken || '') }, secret); const row = new Record($app.findCollectionByNameOrId('karaoke_playlist_confirmations')); set(row, 'token_digest', hash(token)); set(row, 'admin_id', id(auth(c))); set(row, 'playlist_id', playlistId); set(row, 'owner_channel_id', ownerChannelId); set(row, 'visibility', String(preview.visibility || 'public')); set(row, 'snapshot_fingerprint', snapshot); set(row, 'policy_version', policyVersion); set(row, 'etag', String(preview.etag || '')); set(row, 'page_token', String(preview.pageToken || '')); set(row, 'import_cap', Number(preview.importCap || 0)); setJson(row, 'ordered_video_ids_json', preview.orderedVideoIds || []); setJson(row, 'expected_counts_json', preview.expectedCounts || {}); set(row, 'expires_at', future(600000)); set(row, 'status', 'issued'); $app.save(row); return c.json(200, { confirmationToken: token, confirmationExpiresAt: str(row, 'expires_at'), policyVersion, snapshotFingerprint: snapshot }) } catch (_) { return json(c, 409, 'confirmation_issue_failed', 'Preview confirmation could not be issued') }
})
...
routerAdd('POST', '/api/karaoke/tablet/catalog/review/batch', (c) => {
  try { require(__hooks + '/party_queue.pb.js') } catch (_) {}
  const { auth, tablet, json, body, now, id, str, set, setJson, jsonValue, random, catalogApprovalReason } = globalThis.__partyQueue
  if (!tablet(auth(c))) return json(c, 403, 'forbidden', 'tablet_admin authentication required')
  ...
  try {
    ...
    $app.runInTransaction((tx) => {
      ...
      for (const song of songs) { const reason = catalogApprovalReason(song, tx); if (reason) throw new Error(reason) }
  ...
  } catch (error) { const actionable = ['song_not_found', 'already_approved', 'review_state_invalid', 'missing_identity', 'non_karaoke', 'low_confidence', 'alternative_exists', 'unavailable', 'identity_conflict', 'schema_invalid']; const code = actionable.includes(error.message) ? error.message : 'batch_review_failed'; return json(c, code === 'batch_review_failed' ? 500 : code === 'song_not_found' ? 404 : 409, code, code === 'song_not_found' ? 'Song was not found' : code === 'identity_conflict' ? 'Canonical identity conflicts with another song' : code === 'missing_identity' ? 'Canonical artist and title are required' : code === 'non_karaoke' ? 'Only karaoke candidates can be approved' : code === 'unavailable' ? 'The video is unavailable' : code === 'schema_invalid' ? 'Catalog metadata is invalid' : 'Selected song is not approvable') }
})

// Server-scoped selection snapshot: browser submits only source/filter intent.
routerAdd('POST', '/api/karaoke/tablet/catalog/review/selection', (c) => {
  try { require(__hooks + '/party_queue.pb.js') } catch (_) {}
  const { auth, tablet, json, body, id, str, num, hash, now, future, set, setJson, jsonValue } = globalThis.__partyQueue
  if (!tablet(auth(c))) return json(c, 403, 'forbidden', 'tablet_admin authentication required')
  const input = body(c); const source = String(input.source || '').trim(); const filter = input.filter && typeof input.filter === 'object' ? input.filter : {}
  if (!source) return json(c, 422, 'selection_source_required', 'An exact source scope is required')
  try {
    const parser = globalThis.__partyQueue; const playlistScope = /^(?:PL|UU|LL|FL|RD)[A-Za-z0-9_-]{16,}$/.test(source) || /^UC[A-Za-z0-9_-]{20,}:(?:PL|UU|LL|FL|RD)[A-Za-z0-9_-]{16,}$/.test(source); const rows = playlistScope ? $app.findRecordsByFilter('karaoke_songs', 'source = {:sourceType} && source_id = {:sourceId}', '+id', 10000, 0, { sourceType: 'youtube_playlist', sourceId: source }) : $app.findRecordsByFilter('karaoke_songs', 'source = {:source}', '+id', 10000, 0, { source })
    const serverFilter = Object.fromEntries(Object.entries(filter).map(([key, value]) => [key === 'review' ? 'review_status' : key, value]))
    const matches = rows.filter((song) => Object.entries(serverFilter).every(([key, value]) => value === undefined || String(song.get ? song.get(key) : song[key] ?? '') === String(value)))
    const predicate = (song) => !parser.catalogApprovalReason(song, $app)
    const eligible = matches.filter(predicate); const ids = eligible.map((song) => id(song)); const digest = hash(JSON.stringify({ admin: id(auth(c)), source, filter, policy: 'approval-v1', ids })); const collection = $app.findCollectionByNameOrId('karaoke_catalog_selection_snapshots'); let row = $app.findFirstRecordByFilter('karaoke_catalog_selection_snapshots', 'digest = {:digest}', { digest }); if (!row) row = new Record(collection); set(row, 'digest', digest); set(row, 'admin_id', id(auth(c))); set(row, 'source_scope', source); setJson(row, 'filter_json', filter); setJson(row, 'record_ids_json', ids); setJson(row, 'record_versions_json', eligible.map((song) => ({ id: id(song), updatedAt: str(song, 'updated') }))); set(row, 'policy_version', 'approval-v1'); set(row, 'source_filter_digest', digest); set(row, 'expires_at', future(600000)); set(row, 'status', 'issued'); $app.save(row)
    return c.json(200, { digest, source, filter: serverFilter, selectedCount: ids.length, excludedCount: matches.length - ids.length, excludedReasons: { ineligible: matches.length - ids.length }, ids, recordIds: ids, expiresAt: str(row, 'expires_at'), policyVersion: 'approval-v1' })
  } catch (_) { return json(c, 503, 'selection_snapshot_failed', 'Selection snapshot could not be created') }
})

routerAdd('PATCH', '/api/karaoke/tablet/catalog/review/selection', (c) => {
  try { require(__hooks + '/party_queue.pb.js') } catch (_) {}
  const { auth, tablet, json, body, id, str, setJson } = globalThis.__partyQueue
  if (!tablet(auth(c))) return json(c, 403, 'forbidden', 'tablet_admin authentication required')
  const input = body(c); const digest = String(input.digest || ''); const ids = Array.isArray(input.recordIds) ? [...new Set(input.recordIds.map(String))] : []
  if (!/^[a-f0-9]{64}$/i.test(digest) || ids.length > 10000) return json(c, 422, 'selection_update_invalid', 'Selection update is invalid')
  try { let result = null; $app.runInTransaction((tx) => { const snap = tx.findFirstRecordByFilter('karaoke_catalog_selection_snapshots', 'digest = {:digest}', { digest }); if (!snap || str(snap, 'admin_id') !== id(auth(c)) || str(snap, 'status') !== 'issued') throw new Error('selection_snapshot_stale'); const original = jsonValue(snap, 'record_ids_json', []); const allowed = new Set(Array.isArray(original) ? original.map(String) : []); if (ids.some((value) => !allowed.has(value))) throw new Error('selection_scope_mismatch'); const nextDigest = globalThis.__partyQueue.digest(JSON.stringify({ admin: id(auth(c)), source: str(snap, 'source_scope'), filter: jsonValue(snap, 'filter_json', {}), policy: str(snap, 'policy_version'), ids })); const existing = tx.findFirstRecordByFilter('karaoke_catalog_selection_snapshots', 'digest = {:next}', { next: nextDigest }); if (existing && id(existing) !== id(snap)) { result = existing; return } set(snap, 'digest', nextDigest); set(snap, 'source_filter_digest', nextDigest); setJson(snap, 'record_ids_json', ids); tx.save(snap); result = snap }); return c.json(200, { digest: str(result, 'digest'), selectedCount: ids.length, recordIds: ids }) } catch (error) { return json(c, 409, error.message === 'selection_scope_mismatch' ? 'selection_scope_mismatch' : 'selection_snapshot_stale', 'Selection snapshot is stale or outside its source scope') }
})

routerAdd('POST', '/api/karaoke/tablet/catalog/review/selection/commit', (c) => {
  try { require(__hooks + '/party_queue.pb.js') } catch (_) {}
  const { auth, tablet, json, body, id, str, set, setJson, jsonValue, now } = globalThis.__partyQueue
  if (!tablet(auth(c))) return json(c, 403, 'forbidden', 'tablet_admin authentication required')
  const input = body(c); const digest = String(input.digest || ''); const operationId = String(input.operationId || ''); if (!/^[a-f0-9]{64}$/i.test(digest) || !/^[A-Za-z0-9_-]{8,120}$/.test(operationId)) return json(c, 422, 'selection_digest_invalid', 'Selection snapshot is invalid')
  try {
    const parser = globalThis.__partyQueue
    let approved = 0; let excluded = []; let cursor = 0; let complete = false; let authoritative = null
    $app.runInTransaction((tx) => { let op = null; try { op = tx.findFirstRecordByFilter('karaoke_catalog_approval_operations', 'operation_id = {:operationId}', { operationId }) } catch (_) {} if (op && (str(op, 'admin_id') !== id(auth(c)) || str(op, 'selection_digest') !== digest)) throw new Error('selection_operation_binding_mismatch'); if (op && str(op, 'status') === 'complete') { approved = Number(op.get ? op.get('approved_count') : 0); excluded = jsonValue(op, 'excluded_json', []) || []; cursor = Number(op.get ? op.get('cursor') : 0); complete = true; authoritative = op; return } const snap = tx.findFirstRecordByFilter('karaoke_catalog_selection_snapshots', 'digest = {:digest}', { digest }); if (!snap || str(snap, 'admin_id') !== id(auth(c)) || ['committed', 'expired'].includes(str(snap, 'status')) || new Date(str(snap, 'expires_at')).getTime() < Date.now()) throw new Error('selection_snapshot_stale'); const ids = jsonValue(snap, 'record_ids_json', []); const scopedSource = str(snap, 'source_scope'); const playlistScope = /^(?:PL|UU|LL|FL|RD)[A-Za-z0-9_-]{16,}$/.test(scopedSource) || /^UC[A-Za-z0-9_-]{20,}:(?:PL|UU|LL|FL|RD)[A-Za-z0-9_-]{16,}$/.test(scopedSource); if (!op) { op = new Record(tx.findCollectionByNameOrId('karaoke_catalog_approval_operations')); set(op, 'operation_id', operationId); set(op, 'selection_digest', digest); set(op, 'admin_id', id(auth(c))); set(op, 'status', 'running'); set(op, 'cursor', 0); set(op, 'approved_count', 0); setJson(op, 'excluded_json', []); setJson(op, 'audit_json', []); tx.save(op) } cursor = Number(op.get ? op.get('cursor') : 0); excluded = jsonValue(op, 'excluded_json', []) || []; const chunk = Array.isArray(ids) ? ids.slice(cursor, cursor + 20) : []; for (const songId of chunk) { const song = tx.findRecordById('karaoke_songs', String(songId)); const metadata = song ? (jsonValue(song, 'metadata_json', {}) || {}) : {}; let collision = []; if (song) try { collision = tx.findRecordsByFilter('karaoke_songs', 'identity_key = {:identity} && id != {:id}', '', 1, 0, { identity: str(song, 'identity_key'), id: id(song) }) } catch (_) {} const valid = song && (playlistScope ? str(song, 'source') === 'youtube_playlist' && str(song, 'source_id') === scopedSource : str(song, 'source') === scopedSource) && parser.catalogApprovalReason(song, tx) === null ; if (!valid) { excluded.push(String(songId)); continue } const history = jsonValue(song, 'review_history_json', []); const events = Array.isArray(history) ? history : []; events.push({ action: 'selection_approval', digest, operationId, by: id(auth(c)), at: now() }); setJson(song, 'review_history_json', events); set(song, 'review_status', 'approved'); set(song, 'eligible', true); set(song, 'reviewed_by', id(auth(c))); set(song, 'reviewed_at', now()); tx.save(song); approved++ } cursor += chunk.length; set(op, 'cursor', cursor); set(op, 'approved_count', Number(op.get ? op.get('approved_count') : 0) + approved); setJson(op, 'excluded_json', excluded); complete = cursor >= (Array.isArray(ids) ? ids.length : 0); if (complete) { set(op, 'status', 'complete'); set(snap, 'status', 'committed'); tx.save(snap) } tx.save(op); authoritative = op }); return c.json(200, { digest, operationId, approved: Number(authoritative?.get ? authoritative.get('approved_count') : approved), excluded, cursor, complete })
  } catch (error) { return json(c, 409, error.message === 'selection_snapshot_stale' ? 'selection_snapshot_stale' : 'selection_commit_failed', 'Selection snapshot is stale or could not be committed') }
})

routerAdd('POST', '/api/karaoke/tablet/catalog/{id}/review', (c) => {
  try { require(__hooks + '/party_queue.pb.js') } catch (_) {}
  const { auth, tablet, json, body, now, id, str, set, setJson, jsonValue, catalogApprovalReason } = globalThis.__partyQueue
  if (!tablet(auth(c))) return json(c, 403, 'forbidden', 'tablet_admin authentication required')
  ...
  try {
    let result = null
    $app.runInTransaction((tx) => { const song = tx.findRecordById('karaoke_songs', c.request.pathValue('id')); if (!song) throw new Error('song_not_found'); if (reviewState === 'approved') { const reason = catalogApprovalReason(song, tx); if (reason) throw new Error(reason) } const history = jsonValue(song, 'review_history_json', []); if (!Array.isArray(history)) throw new Error('schema_invalid'); const events = history; events.push({ action: 'review', state: reviewState, note: input.note === undefined ? '' : String(input.note).slice(0, 240), by: id(auth(c)), at: now() }); setJson(song, 'review_history_json', events); set(song, 'review_status', reviewState); set(song, 'reviewed_at', now()); set(song, 'reviewed_by', id(auth(c))); if (input.note !== undefined) set(song, 'eligibility_reason', String(input.note).slice(0, 240)); set(song, 'eligible', reviewState === 'approved'); tx.save(song); result = { id: id(song), reviewState, eligible: song.getBool ? song.getBool('eligible') : false } })
    return c.json(200, result)
  } catch (error) { const actionable = ['song_not_found', 'already_approved', 'review_state_invalid', 'missing_identity', 'non_karaoke', 'low_confidence', 'alternative_exists', 'unavailable', 'identity_conflict', 'schema_invalid']; const code = actionable.includes(error.message) ? error.message : 'review_failed'; return json(c, code === 'review_failed' ? 500 : code === 'song_not_found' ? 404 : 409, code, code === 'song_not_found' ? 'Song was not found' : code === 'identity_conflict' ? 'Canonical identity conflicts with another song' : code === 'missing_identity' ? 'Canonical artist and title are required' : code === 'non_karaoke' ? 'Only karaoke candidates can be approved' : code === 'unavailable' ? 'The video is unavailable' : code === 'schema_invalid' ? 'Catalog metadata is invalid' : 'Song is not approvable') }
})
...
routerAdd('POST', '/api/karaoke/tablet/catalog/musicbrainz/match', (c) => {
  ...
  try {
    for (let cursor = startCursor; cursor < ids.length; cursor++) { const songId = ids[cursor]
      ...
      results.push({ id: songId, decision: match.decision, reason: match.reason, confidence: match.confidence || 0, consensus: match.consensus || null, runnerUp: match.runnerUp || null, recording: match.recording || null, provenance: match.recording ? { query: `recording:"${parsed.title}" AND artist:"${parsed.artist}"`, candidateRecordingIds: match.consensus?.recordingIds || [match.recording.id].filter(Boolean), releases: match.recording.releases || [], source: 'musicbrainz' } : null }); processed++
      if (!dryRun && match.decision === 'matched') try { $app.runInTransaction((tx) => { const current = tx.findRecordById('karaoke_songs', songId); if (!current || str(current, 'identity_status') !== 'missing') return; correctCatalogIdentity(tx, current, { title: match.recording.title, artist: match.recording.artist, reason: match.reason, actorId: id(auth(c)), at: now(), source: 'musicbrainz', recordingId: match.recording.id, runnerUp: match.runnerUp || null, matchReason: match.reason, matchConfidence: match.confidence || 0, provenance: { candidateRecordingIds: match.consensus?.recordingIds || [match.recording.id].filter(Boolean), consensus: match.consensus || null, releases: match.recording.releases || [], source: 'musicbrainz' } }) }) } catch (error) { if (error.message === 'identity_conflict') { results[results.length - 1].decision = 'deferred'; results[results.length - 1].reason = 'canonical_collision' } else throw error }
...
// Operator-assumed legacy playlist reconciliation. Rows are selected by the server and
...
function legacySleep(ms) {
  try { if (typeof sleep === 'function') sleep(ms) } catch (_) {}
}
...
function legacyScopeRows(q) {
  ...
  return songs.map((song) => ({
    ...
    videoTitle: q.str(song, 'video_title'),
    playlistPosition: q.num(song, 'playlist_position'),
    source: q.str(song, 'source'),
    playlistSourceId: q.str(song, 'playlist_source_id'),
    reviewStatus: q.str(song, 'review_status'),
    identityStatus: q.str(song, 'identity_status'),
  }))
}

function legacyAcquireRate(q) {
  let acquired = false
  try {
    $app.runInTransaction((tx) => {
      let gate = null
      try { gate = tx.findFirstRecordByFilter('karaoke_musicbrainz_cache', 'cache_key = {:key}', { key: '__rate__' }) } catch (_) {}
      const until = gate && q.str(gate, 'expires_at') ? Date.parse(q.str(gate, 'expires_at')) : 0
      if (until > Date.now()) throw new Error('rate_limited')
      if (!gate) { gate = new Record(tx.findCollectionByNameOrId('karaoke_musicbrainz_cache')); q.set(gate, 'cache_key', '__rate__') }
      q.set(gate, 'updated_at', q.now()); q.set(gate, 'expires_at', q.future(1000)); tx.save(gate); acquired = true
    })
  } catch (error) { if (error.message !== 'rate_limited') throw error }
  return acquired
}

function legacyFetch(q, matcher, queryText) {
  const response = $http.send({ url: `${matcher.MB_ROOT}/recording?query=${encodeURIComponent(queryText)}&fmt=json&limit=10`, method: 'GET', headers: { Accept: 'application/json', 'User-Agent': matcher.USER_AGENT }, timeout: 15 })
...
function legacySaveJob(q, job, token, update) {
  let saved = false
  try { $app.runInTransaction((tx) => { const current = tx.findRecordById('karaoke_legacy_playlist_jobs', q.id(job)); if (!current || q.str(current, 'lease_token') !== token) throw new Error('lease_lost'); update(current); tx.save(current); saved = true }) } catch (_) {}
  return saved
}

function legacyReportSummary(report) {
  const rows = Array.isArray(report) ? report : []
  const reasons = {}
  for (const row of rows) reasons[row.reason || row.decision || 'unknown'] = (reasons[row.reason || row.decision || 'unknown'] || 0) + 1
  return {
    processed: rows.length,
    corrected: rows.filter((row) => row.decision === 'matched').length,
    deferred: rows.filter((row) => row.decision === 'deferred').length,
    replayed: rows.filter((row) => row.decision === 'replay').length,
    musicBrainzRequests: rows.filter((row) => row.cacheHit === false).length,
    cacheHits: rows.filter((row) => row.cacheHit === true).length,
    reasons,
  }
}

function legacyRunJob() {
  try { require(__hooks + '/party_queue.pb.js') } catch (_) {}
  const q = globalThis.__partyQueue
  let job = null
  try { job = $app.findFirstRecordByFilter('karaoke_legacy_playlist_jobs', 'status != {:complete} && policy_version = {:policy}', { complete: 'complete', policy: q.legacyPolicyVersion }) } catch (_) { return }
  if (!job || (q.str(job, 'lease_expires_at') && Date.parse(q.str(job, 'lease_expires_at')) > Date.now())) return
  const token = `${Date.now()}-${q.hash(String(Math.random()))}`
  try {
    $app.runInTransaction((tx) => {
  ...
  try {
    ...
    for (let end = Math.min(rows.length, cursor + q.legacyBatchSize); cursor < end; cursor++) {
      ...
      try {
        ...
        const parsed = matcher.parseYouTubeTitle(q.str(song, 'video_title')); if (parsed.status !== 'parsed') throw new Error(parsed.reason || 'title_unparsed')
        ...
        $app.runInTransaction((tx) => {
          const current = tx.findRecordById('karaoke_songs', row.id)
          ...
          q.correctCatalogIdentity(tx, current, { title: match.recording.title, artist: match.recording.artist, reason: match.reason, actorId: q.str(job, 'initiated_by'), at: q.now(), source: 'musicbrainz', recordingId: match.recording.id, runnerUp: match.runnerUp || null, matchReason: match.reason, matchConfidence: match.confidence || 0, provenance: { bindingKind: q.legacyBindingKind, policyVersion: q.legacyPolicyVersion, playlistId: q.legacyPlaylistId, inputDigest: q.str(job, 'input_digest'), rowDigest, query: queryText, candidateRecordingIds: match.consensus?.recordingIds || [match.recording.id].filter(Boolean), consensus: match.consensus || null, releases: match.recording.releases || [], source: 'musicbrainz' } })
        ...
        if (!applied) { outcome.reason = driftReason || 'input_drift' } else { outcome.decision = 'matched'; outcome.reason = match.reason; outcome.confidence = match.confidence || 0; outcome.recordingId = match.recording.id; outcome.candidateRecordingIds = match.consensus?.recordingIds || [match.recording.id].filter(Boolean); outcome.consensus = match.consensus || null; outcome.runnerUp = match.runnerUp || null }
      } catch (error) { outcome.reason = error.message === 'identity_conflict' ? 'canonical_collision' : error.message || 'deferred' }
      report.push(outcome); if (!legacySaveJob(q, job, token, (current) => { q.set(current, 'cursor', cursor + 1); q.setJson(current, 'report_json', report); q.set(current, 'updated_at', q.now()) })) break
    }
    legacySaveJob(q, job, token, (current) => { q.set(current, 'status', q.num(current, 'cursor') >= rows.length ? 'complete' : 'pending'); q.set(current, 'lease_token', ''); q.set(current, 'lease_expires_at', ''); q.setJson(current, 'report_json', report) })
  } catch (error) { legacySaveJob(q, job, token, (current) => { q.set(current, 'status', 'failed'); q.set(current, 'last_error', String(error.message || 'job_failed').slice(0, 160)); q.set(current, 'lease_token', ''); q.set(current, 'lease_expires_at', '') }) }
}
