...
globalThis.__partyQueue = { CODE_ALPHABET, YOUTUBE_ID, PARTY_TTL, REQUEST_GAP, JOIN_WINDOW, JOIN_LIMIT, PARTY_REQUEST_LIMIT, FALLBACK_QUERY_MAX, FALLBACK_CANDIDATE_MAX, FALLBACK_GUEST_LIMIT, FALLBACK_POLICY_VERSION, CONTROLLER_STATE_TTL, TRUSTED_PLAYLIST_ELIGIBILITY_POLICY, joinAttempts, fallbackAttempts, info, body, auth, bearer, query, requireGuest, activeParty, tablet, hash, normalizeJsonValue, serializeJson, canonicalize, catalogFinalDigest, normalized, fallbackQuery, catalogSafeSong, classifyCatalogItem, recordYoutubeOperation, env, youtubeRequest, fetchYoutubeCandidates, random, code, now, filterDate, future, dayKey, str, num, set, setJson, jsonValue, requiredJsonValue, claimTransitionAllowed, validateClaimReplay, sameInstant, catalogBatchMismatch, id, json, songView, tabletQueueView, tabletControllerView, find, records, chooseNext, catalogImportFailureStage, logCatalogImportFailure, catalogCheckpointHealth, catalogFieldType, findPlaylistSnapshot, classifyTrustedVideoAvailability }
...
routerAdd('POST', '/api/karaoke/tablet/catalog/playlists/import', (c) => {
  ...
  if (!sourceKey && (input.playlistUrl || arbitraryPlaylistId)) {
    let parsed
    try {
      const parser = require(__hooks + '/../catalog/trusted-playlists.cjs')
  ...
  if (!dryRun) {
    try {
      const parser = require(__hooks + '/../catalog/trusted-playlists.cjs'); const secret = String($os.getenv('PLAYLIST_CONFIRMATION_SECRET') || $os.getenv('YOUTUBE_API_KEY') || '')
...
routerAdd('POST', '/api/karaoke/tablet/catalog/review/selection', (c) => {
  ...
  try {
    const parser = require(__hooks + '/../catalog/trusted-playlists.cjs'); const playlistScope = /^(?:PL|UU|LL|FL|RD)[A-Za-z0-9_-]{16,}$/.test(source) || /^UC[A-Za-z0-9_-]{20,}:(?:PL|UU|LL|FL|RD)[A-Za-z0-9_-]{16,}$/.test(source); const rows = playlistScope ? $app.findRecordsByFilter('karaoke_songs', 'source = {:sourceType} && source_id = {:sourceId}', '+id', 10000, 0, { sourceType: 'youtube_playlist', sourceId: source }) : $app.findRecordsByFilter('karaoke_songs', 'source = {:source}', '+id', 10000, 0, { source })
