...
routerAdd('POST', '/api/karaoke/tablet/catalog/import', (c) => {
  try { require(__hooks + '/party_queue.pb.js') } catch (_) {}
  const { auth, tablet, json, body, now, future, find, set, num, hash, canonicalize, catalogFinalDigest, normalized, classifyCatalogItem, fetchYoutubeCandidates, dayKey, random, id, str, catalogImportFailureStage, logCatalogImportFailure } = globalThis.__partyQueue
  if (!tablet(auth(c))) return json(c, 403, 'forbidden', 'tablet_admin authentication required')
  ...
  try {
    ...
    $app.runInTransaction((tx) => {
      ...
      for (let itemIndex = 0; itemIndex < items.length; itemIndex++) {
        ...
        if (song) {
          ...
          if (['missing', 'uncertain'].includes(status)) {
            const history = song.get ? (song.get('review_history_json') || []) : (song.review_history_json || []); const events = Array.isArray(history) ? history : []
            ...
            set(song, 'review_history_json', events); set(song, 'eligible', false); set(song, 'review_status', 'needs_review'); set(song, 'identity_reason', 'canonical_source_identity_proposed'); tx.save(song)
          }
        ...
        if (identity) { const alternatives = identity.get ? (identity.get('alternatives_json') || []) : (identity.alternatives_json || []); const list = Array.isArray(alternatives) ? alternatives : []; if (!list.some((candidate) => String(candidate.youtubeId || '') === youtubeId)) { list.push({ youtubeId, videoTitle: String(item.videoTitle || ''), videoChannelTitle: String(item.channelTitle || ''), videoChannelId: String(item.channelId || ''), canonicalTitle: title, canonicalArtist: artist, classification: result.classification, classificationConfidence: result.confidence, classificationReason: result.reason, candidateRank: Number(item.candidateRank || 0), provenance: live ? 'youtube_api_import' : 'fixture_import', sourceId: String(item.sourceId || ''), sourceList: String(item.sourceList || ''), sourceRank: Number(item.sourceRank || 0), sourceUrl, sourceRetrievedAt: sourceRetrievedAt || now(), importedAt: now() }); set(identity, 'alternatives_json', list); tx.save(identity) } continue }
        set(song, 'youtube_id', youtubeId); set(song, 'title', title || youtubeId); set(song, 'artist', artist); set(song, 'provenance', live ? 'youtube_api_import' : 'fixture_import'); set(song, 'eligibility_reason', identityComplete ? result.reason : 'missing_canonical_identity'); set(song, 'source', String(item.source || (live ? 'listenbrainz' : 'fixture')).slice(0, 80)); set(song, 'source_query', String(input.query || source.terms || '').slice(0, 160)); set(song, 'source_url', sourceUrl); set(song, 'source_retrieved_at', sourceRetrievedAt || now()); set(song, 'source_rank', Number(item.sourceRank || offset + itemIndex + 1)); set(song, 'source_terms', sourceTerms); set(song, 'source_id', String(item.sourceId || '').slice(0, 160)); set(song, 'source_list', String(item.sourceList || '').slice(0, 120)); set(song, 'source_popularity', Number(item.sourcePopularity || 0)); set(song, 'genres_json', Array.isArray(item.genres) ? item.genres : []); set(song, 'release_year', Number(item.releaseYear || 0)); set(song, 'video_title', String(item.videoTitle || '').slice(0, 500)); set(song, 'video_channel_title', String(item.channelTitle || '').slice(0, 240)); set(song, 'video_channel_id', String(item.channelId || '').slice(0, 120)); set(song, 'identity_status', identityComplete ? 'verified_source' : 'missing'); set(song, 'identity_reason', identityComplete ? 'canonical_source_metadata' : 'missing_canonical_identity'); set(song, 'classification', result.classification); set(song, 'classification_confidence', result.confidence); set(song, 'review_status', identityComplete ? 'unreviewed' : 'needs_review'); set(song, 'eligible', false); set(song, 'normalized_title', normalizedTitle); set(song, 'normalized_artist', normalizedArtist); set(song, 'identity_key', identityKey); set(song, 'alternatives_json', []); set(song, 'review_history_json', []); set(song, 'metadata_json', { videoTitle: item.videoTitle || null, channelTitle: item.channelTitle || null, channelId: item.channelId || null, description: item.description || null, embeddable: item.embeddable === true, privacyStatus: item.privacyStatus || null, uploadStatus: item.uploadStatus || null, duration: item.duration || null, viewCount: item.viewCount || null }); set(song, 'import_batch', batchKey); set(song, 'imported_at', now()); diagnosticStage = 'song_save'; tx.save(song); imported++
      }
      ...
      const cursor = Math.max(num(batch, 'cursor'), offset + items.length); if (cursor >= total) { let all = []; try { const chunks = tx.findRecordsByFilter('karaoke_catalog_import_chunks', 'import = {:import}', '+offset', 1000, 0, { import: id(batch) }); chunks.forEach((part) => { const payload = part.get ? part.get('payload_json') : part.payload_json; if (Array.isArray(payload)) all = all.concat(payload) }) } catch (_) {} const computed = catalogFinalDigest({ url: sourceUrl, terms: sourceTerms, retrievedAt: sourceRetrievedAt }, total, all); if (finalDigest && finalDigest !== computed) throw new Error('final_digest_mismatch'); set(batch, 'final_digest', computed) } set(batch, 'cursor', cursor); set(batch, 'status', cursor >= total ? 'complete' : 'paused'); set(batch, 'updated_at', now()); diagnosticStage = 'batch_finalize'; tx.save(batch)
    })
    return c.json(200, { batchKey, imported })
  } catch (error) { if (!live) logCatalogImportFailure(diagnosticStage, offset, items.length); const mismatch = ['batch_source_mismatch', 'chunk_source_mismatch', 'chunk_out_of_order', 'final_digest_mismatch'].includes(error.message); const extra = !live ? { failureStage: catalogImportFailureStage(diagnosticStage) } : undefined; return json(c, mismatch ? 409 : 500, mismatch ? error.message : 'import_failed', error.message === 'chunk_out_of_order' ? 'Import chunks must be submitted in order' : mismatch ? 'Import input does not match its original manifest' : 'Catalog import failed', extra) }
})
...
routerAdd('POST', '/api/karaoke/tablet/catalog/{id}/identity', (c) => {
  try { require(__hooks + '/party_queue.pb.js') } catch (_) {}
  const { auth, tablet, json, body, now, id, str, set, normalized } = globalThis.__partyQueue
  if (!tablet(auth(c))) return json(c, 403, 'forbidden', 'tablet_admin authentication required')
  ...
  try {
    ...
    $app.runInTransaction((tx) => {
      ...
      set(song, 'review_history_json', events); set(song, 'title', title); set(song, 'artist', artist); set(song, 'normalized_title', normalizedTitle); set(song, 'normalized_artist', normalizedArtist); set(song, 'identity_key', identityKey); set(song, 'identity_status', 'operator_corrected'); set(song, 'identity_reason', reason); set(song, 'review_status', 'needs_review'); set(song, 'eligible', false); set(song, 'reviewed_at', now()); set(song, 'reviewed_by', id(auth(c))); tx.save(song)
...
routerAdd('GET', '/api/karaoke/tablet/catalog/report', (c) => {
  try { require(__hooks + '/party_queue.pb.js') } catch (_) {}
  const { auth, tablet, json, str, num } = globalThis.__partyQueue
  if (!tablet(auth(c))) return json(c, 403, 'forbidden', 'tablet_admin authentication required')
  let songs = []
  ...
  const alternatives = songs.reduce((sum, song) => { const value = song.get ? song.get('alternatives_json') : song.alternatives_json; return sum + (Array.isArray(value) ? value.length : 0) }, 0)
  return c.json(200, {
    total: songs.length,
...
routerAdd('POST', '/api/karaoke/tablet/catalog/{id}/replace', (c) => {
  try { require(__hooks + '/party_queue.pb.js') } catch (_) {}
  const { auth, tablet, json, body, now, id, str, set, YOUTUBE_ID } = globalThis.__partyQueue
  if (!tablet(auth(c))) return json(c, 403, 'forbidden', 'tablet_admin authentication required')
