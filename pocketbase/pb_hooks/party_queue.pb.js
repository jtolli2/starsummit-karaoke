...
routerAdd('POST', '/api/karaoke/tablet/catalog/playlists/import', (c) => {
  ...
  try {
    ...
    importPhase = 'persist_results'; $app.runInTransaction((tx) => {
      ...
      if (snapshot) return
      importPhase = 'snapshot_save'; snapshot = new Record(tx.findCollectionByNameOrId('karaoke_playlist_snapshots')); set(snapshot, 'source_key', sourceKey); set(snapshot, 'policy_version', String(source.policyVersion || 'v1')); set(snapshot, 'page_token', pageToken); set(snapshot, 'next_page_token', String(page.nextPageToken || '')); set(snapshot, 'snapshot_fingerprint', snapshotFingerprint); set(snapshot, 'metadata_digest', metadataDigest); setJson(snapshot, 'ordered_video_ids_json', rows.map((row) => row.youtubeId)); set(snapshot, 'retrieved_at', now()); set(snapshot, 'owner_validated_at', now()); tx.save(snapshot)
