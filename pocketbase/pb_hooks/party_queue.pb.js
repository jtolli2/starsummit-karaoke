...
routerAdd('POST', '/api/karaoke/tablet/catalog/playlists/import', (c) => {
  try { require(__hooks + '/party_queue.pb.js') } catch (_) {}
  const { auth, tablet, json, body, now, future, id, str, num, set, setJson, jsonValue, classifyCatalogItem, recordYoutubeOperation, dayKey, hash, serializeJson, random } = globalThis.__partyQueue
  if (!tablet(auth(c))) return json(c, 403, 'forbidden', 'tablet_admin authentication required')
  const PLAYLIST_OWNER_TTL = 6 * 60 * 60 * 1000
  const input = body(c); let allowlist = []
  ...
  const sourceKey = String(input.sourceKey || ''); const source = allowlist.find((entry) => `${entry.channelId}:${entry.playlistId}` === sourceKey)
  ...
  const maxItems = Math.max(1, Math.min(50, Number(input.maxItems || 25) || 25)); const pageToken = String(input.pageToken || '').slice(0, 240); const expectedSnapshot = String(input.snapshotFingerprint || ''); const dryRun = input.dryRun !== false
  if (!dryRun && !/^[a-f0-9]{64}$/i.test(expectedSnapshot)) return json(c, 422, 'playlist_preview_required', 'Import requires an unchanged trusted-playlist preview')
  let priorSnapshot = null
  if (!dryRun) { try { priorSnapshot = $app.findFirstRecordByFilter('karaoke_playlist_snapshots', 'source_key = {:source} && page_token = {:page} && snapshot_fingerprint = {:fingerprint}', { source: sourceKey, page: pageToken, fingerprint: expectedSnapshot }) } catch (_) {} const validated = priorSnapshot ? new Date(str(priorSnapshot, 'owner_validated_at') || str(priorSnapshot, 'retrieved_at')).getTime() : 0; if (priorSnapshot && validated && Date.now() - validated <= PLAYLIST_OWNER_TTL) return c.json(200, { sourceKey, imported: 0, unavailable: 0, duplicates: 0, replay: true, nextPageToken: str(priorSnapshot, 'next_page_token') || '', snapshotFingerprint: expectedSnapshot }) }
  ...
  const policyVersion = String(source.policyVersion || 'v1'); const claimKey = `playlist:${sourceKey}:${policyVersion}:${pageToken}:${maxItems}`; let ownerToken = ''; let cachedPreview = null
  try {
    $app.runInTransaction((tx) => {
  ...
  const call = (path) => { const response = youtubeResponse($http.send({ url: `https://www.googleapis.com/youtube/v3/${path}&key=${encodeURIComponent(key)}`, method: 'GET', headers: { Accept: 'application/json' }, timeout: 15 })); if (response.status < 200 || response.status >= 300) throw new Error(`youtube_http_${response.status || 'unknown'}`); return response.payload }
  try {
    ...
    const playlist = call(`playlists?part=snippet,status&id=${encodeURIComponent(source.playlistId)}`).items?.[0]
    if (!playlist || playlist.status?.privacyStatus !== 'public' || String(playlist.snippet?.channelId || '') !== String(source.channelId)) throw new Error('playlist_owner_mismatch')
    if (!dryRun && priorSnapshot) { $app.runInTransaction((tx) => { const snapshot = tx.findRecordById('karaoke_playlist_snapshots', id(priorSnapshot)); set(snapshot, 'owner_validated_at', now()); tx.save(snapshot); const claim = tx.findFirstRecordByFilter('karaoke_playlist_claims', 'claim_key = {:key}', { key: claimKey }); const reserved = num(claim, 'reserved_units'); const quota = tx.findFirstRecordByFilter('karaoke_youtube_quota', 'day_key = {:day}', { day: str(claim, 'quota_day_key') }); set(quota, 'reserved', Math.max(0, num(quota, 'reserved') - reserved)); set(quota, 'spent', num(quota, 'spent') + 1); tx.save(quota); set(claim, 'reserved_units', 0); set(claim, 'spent_units', num(claim, 'spent_units') + 1); set(claim, 'status', 'ready'); tx.save(claim); recordYoutubeOperation(tx, 'playlists.list', sourceKey, expectedSnapshot, 1, 1) }); return c.json(200, { sourceKey, imported: 0, unavailable: 0, duplicates: 0, replay: true, ownerRevalidated: true, nextPageToken: str(priorSnapshot, 'next_page_token') || '', snapshotFingerprint: expectedSnapshot }) }
    const page = call(`playlistItems?part=snippet,contentDetails,status&playlistId=${encodeURIComponent(source.playlistId)}&maxResults=${maxItems}${pageToken ? `&pageToken=${encodeURIComponent(pageToken)}` : ''}`)
    ...
    let imported = 0; let unavailable = 0; let duplicates = 0
    $app.runInTransaction((tx) => {
      ...
      for (const row of rows) {
        const video = videos.get(row.youtubeId); const status = video?.status || {}; const snippet = video?.snippet || {}; const embeddable = status.embeddable === true; const publicVideo = status.privacyStatus === 'public' && status.uploadStatus === 'processed'; if (!video || !embeddable || !publicVideo) { unavailable++; continue }
      ...
      for (const [operationClass, calls] of [['playlists.list', 1], ['playlistItems.list', 1], ['videos.list', modeled.videosList]]) { if (calls) recordYoutubeOperation(tx, operationClass, sourceKey, snapshotFingerprint, calls, calls) }
      const claim = tx.findFirstRecordByFilter('karaoke_playlist_claims', 'claim_key = {:key}', { key: claimKey }); if (str(claim, 'owner_token') !== ownerToken) throw new Error('playlist_claim_stale'); const reserved = num(claim, 'reserved_units'); const quota = tx.findFirstRecordByFilter('karaoke_youtube_quota', 'day_key = {:day}', { day: str(claim, 'quota_day_key') }); set(quota, 'reserved', Math.max(0, num(quota, 'reserved') - reserved)); set(quota, 'spent', num(quota, 'spent') + modeled.total); tx.save(quota); set(claim, 'reserved_units', 0); set(claim, 'spent_units', num(claim, 'spent_units') + modeled.total); set(claim, 'status', 'ready'); setJson(claim, 'payload_json', { sourceKey, pageToken, nextPageToken: page.nextPageToken || '', snapshotFingerprint, metadataDigest }); tx.save(claim)
    })
    return c.json(200, { sourceKey, imported, unavailable, duplicates, replay: imported === 0 && duplicates === 0 && unavailable === 0, nextPageToken: page.nextPageToken || '', snapshotFingerprint, metadataDigest, modeledCost: modeled })
  } catch (error) { const message = String(error.message || ''); const upstream = /^youtube_http_(401|403|404|429|500|503)$/.test(message) ? message : ''; const auditCode = message === 'playlist_owner_mismatch' ? 'playlist_owner_mismatch' : message === 'playlist_preview_stale' ? 'playlist_preview_stale' : upstream || 'external_result_ambiguous'; try { $app.runInTransaction((tx) => { const claim = tx.findFirstRecordByFilter('karaoke_playlist_claims', 'claim_key = {:key}', { key: claimKey }); if (claim && str(claim, 'owner_token') === ownerToken) { const reserved = num(claim, 'reserved_units'); const quota = tx.findFirstRecordByFilter('karaoke_youtube_quota', 'day_key = {:day}', { day: str(claim, 'quota_day_key') }); set(quota, 'reserved', Math.max(0, num(quota, 'reserved') - reserved)); set(quota, 'spent', num(quota, 'spent') + reserved); tx.save(quota); set(claim, 'reserved_units', 0); set(claim, 'spent_units', num(claim, 'spent_units') + reserved); set(claim, 'status', 'failed'); set(claim, 'error_code', auditCode); tx.save(claim) } }) } catch (_) {} const code = message === 'playlist_owner_mismatch' ? 'playlist_owner_mismatch' : message === 'playlist_preview_stale' ? 'playlist_preview_stale' : upstream || 'playlist_import_failed'; return json(c, code === 'playlist_owner_mismatch' || code === 'playlist_preview_stale' ? 409 : upstream ? 502 : 503, code, code === 'playlist_owner_mismatch' ? 'Playlist ownership or public visibility no longer matches the allowlist' : code === 'playlist_preview_stale' ? 'Playlist changed since preview; review it again before importing' : upstream ? 'Trusted playlist source is temporarily unavailable' : 'Trusted playlist import failed') }
})
