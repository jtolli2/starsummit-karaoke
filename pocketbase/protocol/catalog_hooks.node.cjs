...
test('fallback search normalizes, bounds, classifies, and caches durable candidates', () => {
  ...
  assert.match(hook, /return c\.json\(replayed \? 200 : 201, result\)/)
  ...
  assert.match(hook, /\['party_expired', 'fallback_candidate_unavailable', 'rate_limited', 'duplicate_song'\]\.includes\(retryReason\)/)
  assert.ok(hook.includes("if (!song) { song = new Record(tx.findCollectionByNameOrId('karaoke_songs'))"))
  assert.match(hook, /set\(song, 'mb_match_status', 'not_attempted'\)/)
  assert.match(hook, /fallback_persistence_unavailable/)
  assert.match(hook, /Fallback request could not be saved; retry shortly/)
})
...
test('identity alternatives and review history are append-only and idempotent', () => {
  ...
  assert.match(hook, /function setJson\(r, f, v\) \{ r\.set\(f, normalizeJsonValue\(v\)\)/)
...
test('catalog JSON decoder preserves native values and parses PocketBase raw wrappers', () => {
  const start = hook.indexOf('function jsonValue(')
  const end = hook.indexOf('function sameInstant(', start)
  ...
  assert.deepEqual(jsonValue({ get: () => [91, 93, 0], getString: () => '[91,93,0]' }, 'field', 'fallback'), [91, 93, 0])
  assert.deepEqual(jsonValue({ get: () => [], getString: () => '{"artist":"Beyoncé","title":"東京"}' }, 'field', 'fallback'), { artist: 'Beyoncé', title: '東京' })
