...
test('party lifecycle authorization and expiry on PocketBase 0.39.7', { skip: !process.env.POCKETBASE_BIN }, async (t) => {
  ...
  assert.equal((await call('/api/karaoke/tablet/active', 'GET')).status, 403)
  const restored = await call('/api/karaoke/tablet/active', 'GET', undefined, tablet.json.token)
  assert.equal(restored.status, 200, JSON.stringify(restored)); assert.equal(restored.json.party.id, created.json.id); assert.equal(restored.json.party.codeHint, created.json.code.slice(-4))
  ...
  const unavailable = await call('/api/karaoke/queue/transition', 'POST', { queueId: reqA.json.id, from: 'queued', to: 'playing' }, tablet.json.token)
  assert.equal(unavailable.status, 409); assert.equal(unavailable.json.error, 'controller_unavailable')
  assert.equal((await call('/api/karaoke/parties/queue', 'GET', undefined, joinA.json.credential)).json.queue.find((item) => item.id === reqA.json.id).status, 'queued')
  const controllerState = await call('/api/karaoke/controllers/state', 'PUT', { sessionId: session.json.id, generation: session.json.generation, connectionState: 'connected', videoId: 'dQw4w9WgXcQ', playerState: 'playing', positionSeconds: 1.25, durationSeconds: 120, lastCommandSequence: 1 }, deviceToken)
  assert.equal(controllerState.status, 200, JSON.stringify(controllerState))
  const savedStates = await call(`/api/collections/controller_state/records?filter=${encodeURIComponent(`device = "${enrolled.json.deviceId}"`)}`, 'GET', undefined, su.json.token)
  assert.equal(savedStates.status, 200, JSON.stringify(savedStates)); assert.equal(savedStates.json.items.length, 1)
  assert.equal((await call(`/api/collections/controller_state/records/${savedStates.json.items[0].id}`, 'PATCH', { observed_at: new Date(Date.now() - 91 * 1000).toISOString() }, su.json.token)).status, 200)
  const staleStatus = await call(`/api/karaoke/tablet/status?partyId=${created.json.id}`, 'GET', undefined, tablet.json.token)
  assert.equal(staleStatus.status, 200); assert.equal(staleStatus.json.controller.connectionState, 'disconnected'); assert.equal(staleStatus.json.controller.state, null)
  const staleStart = await call('/api/karaoke/queue/transition', 'POST', { queueId: reqA.json.id, from: 'queued', to: 'playing' }, tablet.json.token)
  assert.equal(staleStart.status, 409); assert.equal(staleStart.json.error, 'controller_unavailable')
  assert.equal((await call('/api/karaoke/parties/queue', 'GET', undefined, joinA.json.credential)).json.queue.find((item) => item.id === reqA.json.id).status, 'queued')
  assert.equal((await call('/api/karaoke/controllers/state', 'PUT', { sessionId: session.json.id, generation: session.json.generation, connectionState: 'connected', videoId: 'dQw4w9WgXcQ', playerState: 'playing', positionSeconds: 1.25, durationSeconds: 120, lastCommandSequence: 1 }, deviceToken)).status, 200)
  ...
  assert.equal((await call(`/api/karaoke/tablet/status?partyId=${created.json.id}`, 'GET')).status, 403)
  const tabletStatus = await call(`/api/karaoke/tablet/status?partyId=${created.json.id}`, 'GET', undefined, tablet.json.token)
  assert.equal(tabletStatus.status, 200, JSON.stringify(tabletStatus))
  assert.equal(tabletStatus.json.party.id, created.json.id)
  assert.ok(Array.isArray(tabletStatus.json.queue))
  assert.equal(tabletStatus.json.controller.device.id, enrolled.json.deviceId)
  assert.equal(tabletStatus.json.controller.connected, true)
  assert.equal(tabletStatus.json.controller.state.videoId, 'dQw4w9WgXcQ')
  assert.equal(Object.prototype.hasOwnProperty.call(tabletStatus.json.controller.device, 'deviceSecret'), false)
  assert.equal(Object.prototype.hasOwnProperty.call(tabletStatus.json.controller.device, 'email'), false)
  assert.equal(Object.prototype.hasOwnProperty.call(tabletStatus.json.controller, 'session'), false)
  ...
  assert.equal((await call(`/api/collections/karaoke_parties/records/${created.json.id}`, 'PATCH', { expires_at: new Date(Date.now() - 1000).toISOString() }, su.json.token)).status, 200)
  const expiredTransition = await call('/api/karaoke/queue/transition', 'POST', { queueId: reqA.json.id, from: 'playing', to: 'completed' }, tablet.json.token)
  assert.equal(expiredTransition.status, 410); assert.equal(expiredTransition.json.error, 'party_expired')
  assert.equal((await call('/api/collections/karaoke_queue/records/' + reqA.json.id, 'GET', undefined, su.json.token)).json.status, 'playing')
})

test('tablet reorder swaps manual order and rejects stale snapshots on PocketBase 0.39.7', { skip: !process.env.POCKETBASE_BIN }, async (t) => {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), 'karaoke-reorder-pb-')); fs.cpSync(path.join(__dirname, '..', 'pb_migrations'), path.join(root, 'pb_migrations'), { recursive: true }); fs.cpSync(path.join(__dirname, '..', 'pb_hooks'), path.join(root, 'pb_hooks'), { recursive: true })
  const dataDir = path.join(root, 'pb_data'); const bin = process.env.POCKETBASE_BIN; execFileSync(bin, ['migrate', 'up', '--dir', dataDir]); execFileSync(bin, ['superuser', 'upsert', 'reorder@test.invalid', 'CorrectHorseBatteryStaple123!', '--dir', dataDir])
  const port = 19900 + Math.floor(Math.random() * 100); const server = spawn(bin, ['serve', '--dir', dataDir, `--http=127.0.0.1:${port}`], { stdio: 'ignore' }); t.after(() => server.kill('SIGTERM')); const base = `http://127.0.0.1:${port}`; for (let i = 0; i < 50; i++) { try { if ((await fetch(`${base}/api/health`)).ok) break } catch (_) {} await new Promise((r) => setTimeout(r, 100)) }
  ...
  const su = await call('/api/collections/_superusers/auth-with-password', 'POST', { identity: 'reorder@test.invalid', password: 'CorrectHorseBatteryStaple123!' }); const user = await call('/api/collections/users/records', 'POST', { email: 'reorder-tablet@test.invalid', password: 'TabletPassword123!', passwordConfirm: 'TabletPassword123!', role: 'tablet_admin' }, su.json.token); const tablet = await call('/api/collections/users/auth-with-password', 'POST', { identity: 'reorder-tablet@test.invalid', password: 'TabletPassword123!' }); assert.equal(user.status, 200); assert.equal(tablet.status, 200)
  const party = await call('/api/karaoke/parties', 'POST', {}, tablet.json.token); const songs = await Promise.all(['dQw4w9WgXcQ', '9bZkp7q19f0', 'J---aiyznGQ'].map((youtube_id, i) => call('/api/collections/karaoke_songs/records', 'POST', { youtube_id, title: `Reorder ${i}`, artist: 'Test', eligible: true, classification: 'karaoke', review_status: 'approved', identity_status: 'verified_source' }, su.json.token))); const guests = await Promise.all([1, 2, 3].map(() => call('/api/karaoke/parties/join', 'POST', { code: party.json.code }))); const requests = await Promise.all(guests.slice(0, 2).map((guest, i) => call('/api/karaoke/requests', 'POST', { youtubeId: songs[i].json.youtube_id }, guest.json.credential))); requests.forEach((request) => assert.equal(request.status, 201, JSON.stringify(request)))
  const snapshot = await call(`/api/karaoke/tablet/status?partyId=${party.json.id}`, 'GET', undefined, tablet.json.token); assert.equal(snapshot.status, 200); const first = snapshot.json.queue[0]; const second = snapshot.json.queue[1]
  const concurrent = await call('/api/karaoke/requests', 'POST', { youtubeId: songs[2].json.youtube_id }, guests[2].json.credential); assert.equal(concurrent.status, 201, JSON.stringify(concurrent))
  const stale = await call('/api/karaoke/tablet/queue/reorder', 'POST', { partyId: party.json.id, queueId: second.id, direction: 'up', expectedRevision: snapshot.json.queueOrderRevision, expectedDigest: snapshot.json.queueOrderDigest }, tablet.json.token); assert.equal(stale.status, 409); assert.equal(stale.json.error, 'stale_reorder')
  const after = await call(`/api/karaoke/tablet/status?partyId=${party.json.id}`, 'GET', undefined, tablet.json.token); assert.ok(after.json.queue.some((item) => item.id === concurrent.json.id)); assert.equal(after.json.queue[0].id, first.id)
  const freshSecond = after.json.queue.find((item) => item.id === second.id); const moved = await call('/api/karaoke/tablet/queue/reorder', 'POST', { partyId: party.json.id, queueId: freshSecond.id, direction: 'up', expectedRevision: after.json.queueOrderRevision, expectedDigest: after.json.queueOrderDigest }, tablet.json.token); assert.equal(moved.status, 200, JSON.stringify(moved)); assert.equal(moved.json.moved, true)
  const reordered = await call(`/api/karaoke/tablet/status?partyId=${party.json.id}`, 'GET', undefined, tablet.json.token); assert.deepEqual(reordered.json.queue.slice(0, 3).map((item) => item.id), [second.id, first.id, concurrent.json.id])
})
